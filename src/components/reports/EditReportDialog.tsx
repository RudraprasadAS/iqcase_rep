...
export const EditReportDialog = ({ report, open, onOpenChange }: EditReportDialogProps) => {
  const { tables, updateReport, isLoadingTables } = useReports();
  ...
  const getAvailableFields = () => {
    if (!report.table_name || !tables) return [];
    const table = tables.find(t => t.table_name === report.table_name);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <Label>Table: {report.table_name}</Label>
            <p className="text-sm text-muted-foreground">
              Table cannot be changed when editing a report
            </p>
          </div>
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <Button
            ...
            disabled={!name.trim() || selectedFields.length === 0 || updateReport.isPending}
          >
